...
filtered_df <- filtered_df[complete.cases(filtered_df[, c('reference_date', 'report_date')]), ]
...
result_df <- count_combinations(filtered_df)
...
result_df$location <- factor("SP")
result_df$age_group <- factor("00+")
...
c(unname(qtiles), target_date) # stack with data frame, then export to csv at end
