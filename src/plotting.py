...
def plot_entire_confints(dataset, model, n_samples = 200, levels = [0.5, 0.95], weeks = True, xlims = None, random_split = True, test_idcs = None):
    ...
    if not random_split:
        if weeks:
            ...
            plt.axvline(2000, color = "black", label="division train/test", linestyle="--")
    if test_idcs is not None:
        plt.vlines(test_idcs, ymin=0, ymax=1000, linewidth = 0.1, colors = "red", label = "Test set")
    plt.xlabel(f"{'EpiWeeks' if weeks else 'Days'} since start of observation")
    plt.legend()
    ...
    if xlims is not None:
        plt.xlim(xlims)
        plt.savefig(f"../outputs/figures/nowcast_{'week' if weeks else 'day'}_subset_{xlims[0]}_{xlims[1]}")
    else:
        plt.savefig(f"../outputs/figures/nowcast_{'week' if weeks else 'day'}")
    plt.show()
